...
export const FileUploaderContent = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
   ({ children, className, ...props }, ref) => {
      ...
      return (
         <div className="w-full" ref={containerRef} aria-description="content file holder">
            <div
               {...props}
               ref={ref}
               className={cn(
...
>(({ className, index, children, ...props }, ref) => {
   ...
   return (
      <div
         ...
         {...props}
      >
         <div className="flex h-full w-full items-center gap-1.5 p-2 font-medium leading-none tracking-tight">
            {children}
         </div>
         <button
            type="button"
            className="group border-l p-2 text-center"
            onClick={() => removeFileFromSet(index)}
         >
            <span className="sr-only">remove item {index}</span>
            <RemoveIcon className="h-4 w-4 text-destructive dark:text-red-500 duration-200 ease-in-out group-hover:text-red-700" />
         </button>
      </div>
   );
});
